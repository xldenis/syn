...
impl Parse for PredImpl {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(PredImpl {
            spans: <[Span; 3]>::from_spans(&spans),
        })
    }
}

#[cfg(feature = "parsing")]
impl Token for PredImpl {
    fn peek(mut cursor: Cursor) -> bool {
        for (i, ch) in "==>".chars().enumerate() {
            match cursor.punct() {
                Some((punct, rest)) => {
                    if punct.as_char() != ch {
...
#[cfg(feature = "parsing")]
impl private::Sealed for PredImpl {}
